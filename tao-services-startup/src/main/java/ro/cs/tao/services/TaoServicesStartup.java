...
public class TaoServicesStartup extends StartupBase {
    ...
    public void onApplicationEvent(ApplicationEvent event) {
        if (event instanceof ContextRefreshedEvent) {
            ...
            TaoConfigurationProvider.getInstance().setValue(TaoConfigurationProvider.APP_HOME, homeDirectory().toString());
            ...
            logger.info("Web interface is accessible at https://localhost:" + TaoConfigurationProvider.getInstance().getValue("server.port"));
            try {
                ...
                String tempPath = TaoConfigurationProvider.getInstance().getValue("spring.servlet.multipart.location", "/mnt/tao/tmp");
            ...
            backgroundWorker.submit(() -> {
                ...
                if (nodeManager != null) {
                    ...
                    logger.fine(String.format("Topology monitoring not available (DRMAA session factory set to %s",
                                              TaoConfigurationProvider.getInstance().getValue("tao.drmaa.sessionfactory")));
                }
            });
            backgroundWorker.submit(() -> {
        ...
        if (node != null) {
            try {
                ...
                if (master == null) {
                    ...
                    String user = TaoConfigurationProvider.getInstance().getValue("topology.master.user", node.getUserName());
                    master.setUserName(user);
                    String pwd = TaoConfigurationProvider.getInstance().getValue("topology.master.password", node.getUserPass());
